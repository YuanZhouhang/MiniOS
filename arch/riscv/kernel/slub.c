...
void *cache_create(const char *name, size_t size, unsigned int aligns, int flags, void* func(void *))
{
	...
	s->tid = cache_tid++;
	printf("[S] Create New cache: name:%s\t\tsize: %d, align: %d\n", s->name, s->size, s->align);
	return s;
}

void *cache_alloc_pages(struct kmem_cache *cache)
{
//	printf("in cache_alloc_pages\n");
	void *p;
	void *tp;
	struct page *page;
...
void *kmem_cache_alloc(struct kmem_cache *cache)
{
	...
	printf("[S] Slub allocate addr: %lx, size: %d\n", object, cache->size);
	return object;
}
...
void *kmalloc(size_t size)
{
	...
	if(size == 0)
	    ...
	    //    struct kmem_cache* KC = kmem_cache_create(kmem_cache_name[objindex], kmem_cache_objsize[objindex], 8, 0, NULL);
